...
def getParameters():
    ...
    if args.predict_path is None:
        args.predict_path = r"Theorem_predictor\LLM_pred_seq.json"

    # text logic forms
        args.text_logic_form_path = r"Disambiguation_module\disambiguated_text_logic_forms_pred.json"
    # diagram logic forms
        args.diagram_logic_form_path = r"Parser\diagram_parser\PGDP.json"
    args.step_limit = 12 # the maximum search steps
    ...
    if answer is not None:
        ...
        try:
            if all([x is not None for x in value_list]) and \
                ...
                if abs(value_list[gt_id] - answer)<1:
    ...
    for index in lst:
        ...
        if args.enable_predict and predict_table is not None:
            if str_index in predict_table:
                order_lst = predict_table[str_index]['num_seqs']
