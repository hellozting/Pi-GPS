...
import random
import time
from utils import isNumber, hasNumber, heron_triangle_formula, angle_area_formula, sort_points, isAlgebra
import sympy
...
class LogicSolver:
    def __init__(self, logic, target=None):
        ...
        self.function_maps = {}
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.circle_theorem["low_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.parallel_theorem["low_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.single_triangle_theorem["low_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.double_triangle_theorem["low_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.polygon_theorem["low_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.complex_single_triangle_angle_theorem["low_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.circle_theorem["high_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.parallel_theorem["high_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.single_triangle_theorem["high_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.double_triangle_theorem["high_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.polygon_theorem["high_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.complex_single_triangle_angle_theorem["high_theorem"].items()})
        self.function_maps.update({k+len(self.function_maps):v for k,v in self.auxiliary_line["high_theorem"].items()})
        self.step_lst = []
        self.target = None
    ...
    def _triangleEqual(length, angle, original_angle):
        ...
        if sum(length) >= 2 and sum(angle) == 1:
            ...
            for i in range(3):
                if angle[i] == True and (original_angle[i][0] == 90 or original_angle[i+3][0] == 90) and length[i] == True:
                    return True # HL
        return False
    ...
    def _equal(list1, list2):
        return any([pair[0].equals(pair[1]) if isinstance(pair[0], sympy.Basic) else pair[0] == pair[1] for pair in product(list1, list2)])
    ...
    def polygon_anglesum_theorem(self):
        ...
        for penta in Pentagons:
            expr = sum(
                [self.logic.find_angle_measure((penta[i], penta[(i + 1) % 5], penta[(i + 2) % 5]))[0] for i in range(5)])
    ...
    def similar_triangle_proving_theorem(self):
        ...
        for pair in comb:
            for tri1 in permutations(pair[0]):
                ...
                eq = self._equal
                same_angle = [s(angles[0], angles[3]), s(angles[1], angles[4]), s(angles[2], angles[5])]
                same_ratio  = [eq(_mul(length[1], length[5]), _mul(length[2], length[4])), eq(_mul(length[0], length[5]), _mul(length[2], length[3])), eq(_mul(length[0], length[4]), _mul(length[1], length[3]))]
    ...
    def similar_polygon_theorem(self):
        ...
        for poly1, poly2 in polygons:
            length1 = [self.logic.find_line_with_length((poly1[i%len(poly1)], poly1[(i+1)%len(poly1)]))[0] for i in range(len(poly1))]
            length2 = [self.logic.find_line_with_length((poly2[i%len(poly1)], poly2[(i+1)%len(poly2)]))[0] for i in range(len(poly2))]
            angles1 = [self.logic.find_angle_measure((poly1[i%len(poly1)], poly1[(i+1)%len(poly1)], poly1[(i+2)%len(poly1)]))[0] for i in range(len(poly1))]
            angles2 = [self.logic.find_angle_measure((poly2[i%len(poly2)], poly2[(i+1)%len(poly2)], poly2[(i+2)%len(poly2)]))[0] for i in range(len(poly2))]
            Area1 = self.logic.newAreaSymbol('Polygon({})'.format(','.join(sort_points(poly1)))).subs(self.logic.variables)
            Area2 = self.logic.newAreaSymbol('Polygon({})'.format(','.join(sort_points(poly2)))).subs(self.logic.variables)
            ...
            for i in range(len(poly1)):
                ...
                self.equations.append(expr)
                self.logic.PutIntoEqualAngleSet((poly1[i%len(poly1)], poly1[(i+1)%len(poly1)], poly1[(i+2)%len(poly1)]), (poly2[i%len(poly2)], poly2[(i+1)%len(poly2)], poly2[(i+2)%len(poly2)]))
                ...
                if (hasNumber(angleA) and hasNumber(angleB) and (hasNumber(lengthAC) or hasNumber(lengthBC))) or \
                   (hasNumber(lengthAC) and hasNumber(lengthBC) and (hasNumber(angleA) or hasNumber(angleB))):
                    ...
                    if self._hasSymbol(expr):
                        if self.logic.debug:
                            print("Law of Sines: in tri " ,_ , expr)
                        self.equations.append(expr)
                        Update = True
        return Update
            ...
            for _ in permutations(tri):
                ...
                if known_edges == 2:
                    if hasNumber(lengthAB) and hasNumber(lengthAC):
                        Update = self.logic.define_length(*lines[2], (lengthAB[0]**2 - lengthAC[0]**2)**0.5) or Update
                    if hasNumber(lengthAB) and hasNumber(lengthBC):
                        Update = self.logic.define_length(*lines[1], (lengthAB[0]**2 - lengthBC[0]**2)**0.5) or Update
                    if hasNumber(lengthAC) and hasNumber(lengthBC):
                        Update = self.logic.define_length(*lines[0], (lengthAC[0]**2 + lengthBC[0]**2)**0.5) or Update
        ...
        for tri in triangles:
            ...
            for i, line in enumerate(lines):
                for p in self.logic.find_all_points_on_line(line):
                    if p != line[0] and p != line[1] and self._same(self.logic.find_line_with_length((p,line[0])), self.logic.find_line_with_length((p,line[1]))):
                        midpoint_list[i] = p
                        break
    ...
    def area_equation_theorem(self):
        Update = False
        if not (self.target != None and 'Area' in self.target or any([('AreaOf' in str(k)) for k in self.logic.variables.keys()])):
            return Update
        quads = self.logic.find_all_quadrilaterals()
        for quad in quads:
            Area = self.logic.newAreaSymbol('Polygon({})'.format(','.join(sort_points(quad)))).subs(self.logic.variables)
            parallel_pair = []
            ...
            for upper_edge, lower_edge in parallel_pair:
                ...
                for angle in angles:
                    if (angle[1] in upper_edge_points and angle[0] in upper_edge_points and angle[2] in lower_edge_points) or \
                    (angle[1] in lower_edge_points and angle[0] in lower_edge_points and angle[2] in upper_edge_points):
            ...
            for (upper_edge, lower_edge), (leg1, leg2) in zip(parallel_pair, leg_pair):
                ...
                for i, leg in enumerate((leg1, leg2)):
                    for p in self.logic.find_all_points_on_line(leg):
                        if p != leg[0] and p != leg[1] and self._same(self.logic.find_line_with_length((p,leg[0])), self.logic.find_line_with_length((p,leg[1]))):
                            median_line[i] = p
    ...
    def _getAnswer(self, target):
        if self.logic.debug:
            print("in get answer taget:",target)
        """
        Give the target (The format is defined above) and find its answer if possible.
        """
        if target[0] == 'Minus':
            tried1 = self._getAnswer(target[1])
            tried2 = self._getAnswer(target[2])
            if tried1 is not None and tried2 is not None:
                return tried1-tried2
        if target[0] == 'Add':
            tried1 = self._getAnswer(target[1])
            tried2 = self._getAnswer(target[2])
            if tried1 is not None and tried2 is not None:
        ...
        if target[0] == 'Area':
            ...
            if l == 1:
                ...
                for i in range(len(points)):
                    ...
                    print(target[1], points[i],length)
                    if hasNumber(length):
                ...
                area_symbol = self.logic.newAreaSymbol('Polygon({})'.format(','.join(sort_points(target[1:])))).subs(self.logic.variables)
    ...
    def initSearch(self):
        ...
        if self.logic.debug:
            print("Init Search")
            print("angles",self.logic.find_all_angle_measures())
            print("lines",self.logic.find_all_lines_with_length())
            print("arcs",self.logic.fine_all_arc_measures())
    ...
    def Search(self, target, order_list, round_or_step, upper_bound, enable_low_first):
        print(order_list)
        enable_low_first = True

        self.logic.debug = False
        #self.logic.debug = True
        if self.logic.debug:
            print("order list",order_list)
            print("\nin Search ", target)
            print("variables:",self.logic.variables)
        if target is None:
            return None
        ...
        if round_or_step:
            #print("in round upper_bound:",upper_bound)
            Update = True
            while rounds <= upper_bound and Update:
                ...
                while Update:
                    ...
                    for i in range(1, len(self.function_maps)+1):
                        ...
                        if changed is not None and changed:
                            ...
                            Update = True
                            if self.logic.debug:
                                print("\n",self.function_maps[i],"\nvariables:",self.logic.variables)
                            self.Solve_Equations()
                            now_answer = self._getAnswer(target)
                            if now_answer is not None:
                                return now_answer, rounds, step_lst
                for i in range(self.interval+1, len(self.function_maps)+1):
                    ...
                    if changed is not None and changed:
                        ...
                        if now_answer is not None:
                            return now_answer, rounds, step_lst
                if len(step_lst)>=5 and step_lst[-1] == step_lst[-2] and step_lst[-1] == step_lst[-3] and step_lst[-1] == step_lst[-4]:
                    if self.logic.debug:
                        print("break for same step")
                    break
